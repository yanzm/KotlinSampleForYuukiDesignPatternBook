...
package net.yanzm.sample.factorymethod4.improved
...
abstract class Factory<T : Product> {
    ...
    fun create(owner: String): T {
        val p = createProduct(owner)
        registerProduct(p)
        return p
    }

    protected abstract fun createProduct(owner: String): T
    protected abstract fun registerProduct(product: T)
}
...
class IDCardFactory : Factory<IDCard>() {
    ...
    override fun registerProduct(product: IDCard) {
